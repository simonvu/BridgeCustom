...
class FacetsFormComponent extends Component {
   ...
   */
  updateFilters = () => {
    // If defer-submit is enabled (drawer mode), skip auto-submit
    if (this.dataset.deferSubmit === 'true') {
      return;
    }
    this.#executeUpdate();
  };
  ...
  /**
   * Force update - used by Apply button to bypass defer-submit
   */
  forceUpdateFilters = () => {
    this.#executeUpdate();
  };
  ...
  /**
   * Internal: actually perform the filter update
   */
  #executeUpdate() {
    this.#updateURLHash();
...
class FacetInputsComponent extends Component {
  ...
  updateFilters() {
    ...
    // If defer-submit is set (drawer mode), don't auto-submit
    if (facetsForm.dataset.deferSubmit === 'true') {
